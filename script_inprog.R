...
ggplot(memlos_df, aes(x = CIMEMLOS, y = count))+
  ...
  labs(title = "Subjective Cognitive Decline - Minnesota 2015 BRFSS",
       x = "Presence",
       y = "Count")


#MAIN GLM MODEL - SCD based on perceived sprt
df1_lm <- glm(CIMEMLOS ~ EDUCATION + EMTSUPRT + SEX, data = df1_final_comb, family = "binomial")
...
probabilities <- 1 / (1 + exp(coefficients))
print(probabilities)
...
plot_ed <- predictorEffect("EDUCATION", df1_lm)
plot(plot_ed)
